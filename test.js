...
const Server = mongo.Server;    //引用主要的构造函数
const Db = mongo.Db;    //引用主要的构造函数
...
const db = new Db('test', server); //实例化链接，第一个参数是数据库的名称，第二个是 实例化的server对象
...
openC('user',function(result){
  result.findOne(function(err,data){
     if(err){
       console.log(err)
     } else{
       console.log(data)
...
function sqlFun(colName, funName, order){   //增删改查处理所用的方法
  this.fun= function(col_){
    switch(funName){
      case 'findOne':
        col_.findOne(function(err,data){
          if(err){
            console.log(err)
          } else{
            console.log(data)
